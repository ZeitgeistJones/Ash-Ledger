...
async function scanRange(fromBlock, toBlock) {
  ...
  async function scanFor(toTopic) {
    ...
    // (e.g. Incinerator gaps) while scannedTo still advanced past the hole.
    // Callers (stats API) must NOT bump scannedTo when this throws; they should
    // persist the failed window under ash-ledger:scan-gaps:v1 for retry.
    if (hardFailures > 0) {
      throw new Error(
